...
function AppointmentsPage() {
  ...
  const updateStatusMutation = useMutation(() => ({
    mutationFn: (params: { id: string, status: AppointmentStatus }) =>
      dentalOps.appointments.updateAppointmentStatus(params.id, params.status),
    onSuccess: () => {
  ...
  return (
    <div class="max-w-7xl mx-auto px-4 py-6">
      ...
      <Show when={showDetailsModal()}>
        <AppointmentDetails selectedAppointment={selectedAppointment} setShowDetailsModal={setShowDetailsModal} handleStatusUpdate={handleStatusUpdate} updateStatusMutation={updateStatusMutation} />
      </Show>
    </div>
  );
}
